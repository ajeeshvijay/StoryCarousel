class StoryCarousel {
  ...
  animate() {
    clearTimeout(this.startAnimate);
    clearInterval(this.startAnimateInterval);
    this.startAnimate = setTimeout(() => {
      ...
      this._index++;
      this.startAnimateInterval = setInterval(() => {
        this._items.forEach((item, i) => {
          item.classList.remove('story-carousel-item-active');
          this._indicators[i].classList.remove('story-carousel-indicator-active');
          this._indicators[i].querySelector('.story-carousel-indicator-loading').style.transitionDuration = '0ms';
        });
        this._items[this._index].classList.add('story-carousel-item-active');
        this._indicators[this._index].classList.add('story-carousel-indicator-active');
        this._indicators[this._index].querySelector('.story-carousel-indicator-loading').style.transitionDuration = this.opts.duration + 'ms';
        this._index++;
